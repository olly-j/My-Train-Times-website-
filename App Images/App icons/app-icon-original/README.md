...
a rainbow. The train has a **pale glass body** with a **domed roof and a solid, flatter
base** (plus a subtle bumper seam), **one dark-blue windscreen split by a centre
divider** into two panes with a clean glass glint, an abstract **amber "arrivals"
strip** in the destination-panel position, and **two small oval headlamps** low by the
bumper. A soft **shadow halo** lifts the train off the ribbons.
...
- **Flat, frontal, bold.** "A frontal view and a more flat appearance"; avoid
  ...
  with generous radii and no hairlines (the bumper seam is a filled band, not a line).
...
| `layers/03-train-body.svg` | Train body | Pale-glass front, domed roof + solid flatter base + bumper seam |
| `layers/04-train-windows.svg` | Windscreen | One dark-blue windscreen split by a centre divider (two panes) + glass glint |
| `layers/05-train-lights.svg` | Warm accents | Amber "arrivals" strip (high) + two oval headlamps (low) + halos |
...
- **120 px:** train body, the divided windscreen, the amber strip and the two lamps are
  all distinct; the three ribbons read as clean coloured motion trails. ✅
- **60 px:** silhouette holds — clearly a front-facing Tube train; the strip/lamps
  simplify but the divided windscreen + solid body shape still says "train". ✅
...
- **Mono / Tinted:** **verified** by converting the render to luminance — the bright
  body, dark divided windscreen and bright accents give a clean single-tone train; the
  ribbons fall back to soft grey trails. The symbol fully carries recognisability with
  zero colour. ✅
...
- **Train body:** rounded path spanning x=300→724, y=296→772 (centre 512); top radius
  ~96 (domed roof), bottom radius ~40 (solid flatter base). Bumper seam:
  `rect 344,726 336×26 rx13`.
- **Windscreen:** two panes `rect 348,366 156×128 rx26` & `rect 520,366 156×128 rx26`
  (16 px centre divider); double white glint polygons in the left pane.
- **Arrivals strip:** `rect 452,549 120×22 rx11` (amber), centred under the windscreen.
- **Lamps:** ovals at `(404,694)` & `(620,694)`, rx27 ry22 (+ rx48 ry40 halo).
...
- **Palette:** bg `#1E2E3B→#04070C`, body `#FCFEFF→#B9C6D3`, windscreen
  `#1B3654→#091826`, lamps `#FFEAB6→#EE7E16`, arrivals `#FFE08A→#FF9F22`,
